...
const initialFormData = {
    ...
    Price: 0,
    Stock: 0,
    CategoryId:"", // Default value for hidden input
...
console.log(categoryOptions);
    ...
    const showNoProductFound = !loading && Object.keys(product).length === 0;
...
async function  update_product() {
    ...
    if(res.statusCode==200)toast.success("Product updated successfully");
...
async function getCategNames() {
 ...
 setCategoryOptions(categories);
        ...
        const fetchProduct = async () => {
            ...
            try {
                ...
                if (res.statusCode === 200) {
                    ...
                    // Initialize formData with fetched product values
                    setFormData({
    ...
    }, [id]); // Dependency array: run only when 'id' changes
    ...
    const handleSubmit = () => {
        ...
        if (isNameValid && isPriceValid && isStockValid && isDescriptionValid) {
    ...
    if (loading) {
        return <LoadingModal loading={true} text="Loading product details..." />;
    }
    ...
    return (
        <div className="EditProduct DashboardPage">
            ...
            {showNoProductFound ? (
                <LoadingModal loading={false} text="No product found with this ID." />
            ) : (
                        ...
                        <div className="form-group">
                            ...
                            <div className="file-input-wrapper">
                                <input
                                    type="file"
                                    id="uploadImages"
                                    Name="productImages" // Changed Name for simplicity
                                    multiple
                                    accept="image/*"
                                    // File inputs are UNCONTROLLED: do not use the 'value' prop
                                    // You would handle the files using a dedicated state/handler
                                />
                                <span>Click to select images or drag & drop here</span>
                            </div>
                            {/* You could add a section here to show PREVIOUSLY uploaded images */}
                        </div>
